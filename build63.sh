#!/bin/sh
# Build a custom image of the OpenBSD 6.3 release.
#
# Author: Tim Baumgard
...
--mirror
URL of a nearby OpenBSD mirror. It should be in the same format used by installurl(5). This parameter is required.
...
echo "This script uses the /mnt directory. You must unmount any file systems that are currently using this directory."
echo "This script uses vnd0. You must unmount any file systems using vnd0 and unconfigure it before continuing. See vnconfig(8)."
echo "This script compiles and installs rdsetroot from the OpenBSD sources (src.tar.gz) to /usr/local/sbin/rdsetroot if it doesn't already exist."
echo -n "Continue? [y/N] "
read CONTINUE
...
for OPENBSD_SOURCE in ${OPENBSD_SOURCES[@]}; do
	# Skip downloading src.tar.gz and company if rdsetroot already exists.
	if [ -x /usr/local/sbin/rdsetroot ]; then
		if echo "${OPENBSD_SOURCE}" | grep -qv "${ARCH}"; then
			continue
		fi
	fi
...
if [ ! -x /usr/local/sbin/rdsetroot ]; then
	status "Creating rdsetroot executable. This will take awhile."

	mkdir -p "${SOURCES}/${RELEASE}/src"
	(cd "${SOURCES}/${RELEASE}/src" && tar xzf "${SOURCES}/${RELEASE}/src.tar.gz")

	(
		cd "${SOURCES}/${RELEASE}/src/distrib/common"
		cc -o rdsetroot elf32.c elf64.c elfrdsetroot.c
		install -o root -g bin rdsetroot /usr/local/sbin/rdsetroot
	)

	rm -rf "${SOURCES}/${RELEASE}/src"
fi
